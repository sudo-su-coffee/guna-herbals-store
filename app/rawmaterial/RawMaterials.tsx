...
const RawMaterials: React.FC<RawMaterialsProps> = ({ setView }) => {
  useEffect(() => {
      updateMetaTags(
          "Bulk Herbal Raw Materials - B2B Supplier", 
          "Sourcing premium organic ingredients for artisans and businesses. Cold pressed oils, herbal powders, and natural bases from Tamil Nadu."
      );
  }, []);
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-earth-200">
                            ...
                            {materials.map((item, idx) => (
                                <tr key={idx} className="hover:bg-earth-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 md:px-8 md:py-6">
                                        <button 
                                            onClick={() => setView(ViewState.ENQUIRY)} 
                                            className="bg-herbal-800 text-white px-4 py-2 md:px-6 md:py-2 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-widest hover:bg-black transition-all shadow-md whitespace-nowrap"
                                        >
                                            Enquire
                                        </button>
                                    </td>
                ...
                <div className="p-6 md:p-8 bg-earth-50 border-t border-earth-200 text-center">
                    ...
                    <button onClick={() => setView(ViewState.ENQUIRY)} className="text-herbal-800 font-bold underline hover:text-black text-sm md:text-base">Contact Procurement Team</button>
                </div>
            </div>
        </div>
    </div>
