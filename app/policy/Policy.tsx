...
import { PolicyType, ViewState } from '../../types';
import { POLICIES } from '../../constants';

interface PolicyProps {
  activePolicy: PolicyType;
  setActivePolicy: (type: PolicyType) => void;
...
const Policy: React.FC<PolicyProps> = ({ activePolicy, setActivePolicy, setView }) => {
                ...
                <nav className="flex lg:block space-x-2 lg:space-x-0 lg:space-y-1 min-w-max">
                    {MENU_ITEMS.map(item => (
                        <button
                            ...
                            className={`flex-shrink-0 lg:w-full text-left px-4 py-3 md:px-5 md:py-4 rounded-lg text-xs md:text-sm font-bold flex items-center gap-2 md:gap-4 transition-all duration-300 ${
            ...
            <div className="bg-white p-6 md:p-16 rounded-2xl shadow-xl border border-earth-200 min-h-[60vh] relative overflow-hidden">
                ...
                <div className="mt-12 md:mt-16 pt-6 md:pt-8 border-t border-earth-100 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-gray-400">
                    ...
                    <div className="flex gap-4">
                        ...
                        <button onClick={() => setView(ViewState.ENQUIRY)} className="hover:text-herbal-900 font-bold transition-colors">Contact Support</button>
                    </div>
                </div>
            </div>
        </main>
